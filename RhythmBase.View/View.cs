...
(i + i % 2 * (1 - swing) - (i <= (prexs?.SyncoBeat ?? -1) ? 0 : (prexs?.SyncoSwing ?? 0)))), 0));
            ...
            case Comment comment:
                {
                    ...
                    canvas.DrawBack(
                        ...
                        color,
                        style);
                    canvas.DrawSlice(key, SKPoint.Empty);
                }
                break;
            ...
            case ReorderRooms reorderRooms:
                {
                    ...
                    canvas.DrawBack(destRect, ColorOf(evt.Tab), style);
                    SKPoint p = SKPoint.Empty;
                    foreach (var r in reorderRooms.Order.Take(4))
                    {
                        canvas.DrawSlice($"{key}_{r}", p);
                        p.Offset(0, iconSize);
                    }
                }
                break;
            case ReorderWindows reorderWindows:
                {
                    ...
                    canvas.DrawBack(destRect, ColorOf(evt.Tab), style);
                    SKPoint p = SKPoint.Empty;
                    foreach (var r in reorderWindows.Order.Take(4))
                    {
                        canvas.DrawSlice($"{key}_{r}", p);
                        p.Offset(0, iconSize);
                    }
            ...
            case SetRowXs setRowXs:
                {
                    destRect = SKRect.Create(0, 0, info.Bounds.Width, info.Bounds.Height);
                    canvas.DrawBack(destRect, ColorOf(evt.Tab), style);
                    float width = info.Bounds.Width / 6f;
                    ...
                    float left = 0;
                    float top = iconSize / 2f - info2.Bounds.Height * s / 2f;
            ...
            default:
                ...
                switch (evt)
                {
                    case CustomFlash customFlash:
                        canvas.DrawSlice($"{key}_0", SKPoint.Empty, ToSKColor(customFlash.StartColor?.Value ?? RDColor.Transparent));
                        canvas.DrawSlice($"{key}_1", SKPoint.Empty, ToSKColor(customFlash.EndColor?.Value ?? RDColor.Transparent));
                        break;
                    ...
                    case FloatingText floatingText:
                        canvas.DrawSlice($"{key}_0", SKPoint.Empty, ToSKColor(floatingText.Color));
                        canvas.DrawSlice($"{key}_1", SKPoint.Empty, ToSKColor(floatingText.OutlineColor));
                        break;
                    ...
                    case PaintHands paintHands:
                        canvas.DrawSlice(key, SKPoint.Empty, ToSKColor(paintHands.TintColor));
                        switch (paintHands.Border)
                        {
                            case Border.Outline:
                                canvas.DrawSlice($"{key}_0", SKPoint.Empty, ToSKColor(paintHands.BorderColor));
                                break;
                            case Border.Glow:
                                canvas.DrawSlice($"{key}_1", SKPoint.Empty, ToSKColor(paintHands.BorderColor));
                                break;
                        }
                        break;
                    case SetBackgroundColor setBackgroundColor:
                        {
                            canvas.DrawSlice(key, SKPoint.Empty);
                            canvas.DrawSlice($"{key}_0", SKPoint.Empty,
                            ...
                            switch (setBackgroundColor.BackgroundType)
                            {
                                ...
                                case BackgroundType.Image:
                                    ...
                                    if (setBackgroundColor.Images.Count == 0 || !AssetManager._slices.TryGetValue($"{key}_1", out SliceInfo info2))
                                        break;
                                    string path = FilepathOf(Directory, setBackgroundColor.Images[0]);
                                    if (!File.Exists(path))
                                        break;
                    ...
                    case SetForeground setForeground:
                        {
                            ...
                            if (setForeground.Images.Count == 0 || !AssetManager._slices.TryGetValue($"{key}_1", out SliceInfo info2))
                                break;
                            string path = FilepathOf(Directory, setForeground.Images[0]);
                            if (!File.Exists(path))
                                break;
                            SKBitmap bitmap = SKBitmap.Decode(path);
                            SKRect imgDest = SKRect.Create(-info2.Pivot.X, -info2.Pivot.Y, info2.Bounds.Width, info2.Bounds.Height);
                    ...
                    case Tint tint:
                        ...
                        {
                            case Border.Outline:
                                canvas.DrawSlice($"{key}_0", SKPoint.Empty, ToSKColor(tint.BorderColor));
                                break;
                            case Border.Glow:
                                canvas.DrawSlice($"{key}_1", SKPoint.Empty, ToSKColor(tint.BorderColor));
                                break;
                        }
                        break;
                    case TintRows tintRows:
                        canvas.DrawSlice(key, SKPoint.Empty, ToSKColor(tintRows.TintColor));
                        switch (tintRows.Border)
                        {
                            case Border.Outline:
                                canvas.DrawSlice($"{key}_0", SKPoint.Empty, ToSKColor(tintRows.BorderColor));
                                break;
                            case Border.Glow:
                                canvas.DrawSlice($"{key}_1", SKPoint.Empty, ToSKColor(tintRows.BorderColor));
                                break;
                        }
                        break;
                    default:
        ...
        if (evt.Condition.HasValue)
        {
            ...
            if (hasTrue)
                if (hasFalse)
                    canvas.DrawSlice(evtag, SKPoint.Empty, 0xffffff00);
                else
                    canvas.DrawSlice(evtag, SKPoint.Empty, 0xff00ffff);
            else if (hasFalse)
                canvas.DrawSlice(evtag, SKPoint.Empty, 0xffff0000);
        }
    ...
    public static RDColor[] Palette
    {
        get => field;
        set
        {
            if (value.Length != 21)
                throw new ArgumentException("Palette must have exactly 21 colors.");
            field = value;
        }
    }
    public static string Directory { get; set; } = string.Empty;
    private static SKColor ToSKColor(PaletteColor color)
    {
        if (color.EnablePanel)
            return ToSKColor(View.Palette[color.PaletteIndex]);
        return ToSKColor(color.Value);
    }
    private static SKColor ToSKColor(PaletteColorWithAlpha color)
    {
        if (color.EnablePanel)
            return ToSKColor(View.Palette[color.PaletteIndex]);
        return ToSKColor(color.Value);
    ...
    private static string FilepathOf(string directory, string relativePath)
    {
        if (Path.IsPathRooted(relativePath))
            return relativePath;
        else
            return Path.Combine(directory, relativePath ?? string.Empty);
    }
